...
module.exports = function(options) {
  ...
  var gameHTML = `
    ...
    <script>
      ...
      for(var i=0;i<9;i++){var btn=document.createElement('div');btn.style.cssText='width:120px;height:120px;background:#3498db;border-radius:12px;cursor:pointer;transition:all 0.2s';btn.dataset.index=i;btn.addEventListener('click',function(){if(!playing)return;var idx=parseInt(this.dataset.index);this.style.background='#2980b9';setTimeout(function(){document.querySelectorAll('#pattern-grid div')[idx].style.background='#3498db';}.bind(this),200);playerPattern.push(idx);var lang=window.currentLang||'ko';var txt=window.i18n[lang].games.pattern;if(playerPattern[playerPattern.length-1]!==pattern[playerPattern.length-1]){patternResult.textContent=txt.wrong;patternResult.style.color='#e74c3c';playing=false;var pmIsNew=window.GameRecord.save('pattern-memory','level',Math.max(level-1,0));if(historyVisible)renderHistory();var lang=window.currentLang||'ko';var txt2=window.i18n[lang].games.pattern;var fl=Math.max(level-1,0);var pmMsg=fl>=12?txt2.msg1:fl>=9?txt2.msg2:fl>=7?txt2.msg3:fl>=5?txt2.msg4:txt2.msg5;patternResult.textContent=txt2.wrong+pmMsg;if(pmIsNew)patternResult.textContent+=(window.i18n[lang].newRecord||' 🎉 신기록!');patternResult.style.color='#e74c3c';var shtxt=lang==='ko'?'🧩 패턴 기억 레벨 '+fl+' 달성! 너도 해봐!':lang==='ja'?'🧩 パターン記憶レベル'+fl+'達成！あなたも挑戦！':'🧩 I reached level '+fl+' in pattern memory! Beat me!';window._shareResult={title:shtxt,text:shtxt,url:window.location.href};level=1;patternLevelEl.textContent=level;patternStart.style.display='inline-block';}else if(playerPattern.length===pattern.length){var completedLevel=level;playing=false;level++;patternLevelEl.textContent=level;if(completedLevel>bestLevel){bestLevel=completedLevel;patternBestEl.textContent=bestLevel;window.GameRecord.save('pattern-memory','level',bestLevel);}patternResult.textContent=txt.correct;patternResult.style.color='#27ae60';setTimeout(showPattern,1500);}});patternGrid.appendChild(btn);}
