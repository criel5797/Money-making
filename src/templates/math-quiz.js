...
module.exports = function(options) {
  ...
  var gameHTML = `
    ...
    <script>
      ...
      function startGame(){timeLeft=60;correct=0;correctEl.textContent=correct;mathInput.disabled=false;mathInput.value='';mathInput.focus();mathStart.style.display='none';mathResult.textContent='';generateQuestion();timerInterval=setInterval(function(){timeLeft--;timerEl.textContent=timeLeft;if(timeLeft<=0){clearInterval(timerInterval);mathInput.disabled=true;var lang=window.currentLang||'ko';var txt=window.i18n[lang].games.math;mathQuestion.textContent=txt.timeUp;var mathIsNew=window.GameRecord.save('math-quiz','correct',correct);if(correct>bestCorrect){bestCorrect=correct;bestCorrectEl.textContent=bestCorrect;}var mathMsg=correct>=25?txt.msg1:correct>=20?txt.msg2:correct>=15?txt.msg3:correct>=10?txt.msg4:correct>=7?txt.msg5:txt.msg6;var sharePerc=correct>=25?' 상위 1%!':correct>=20?' 상위 5%!':correct>=15?' 상위 20%!':'';var shtxt=lang==='ko'?'🧮 암산 60초에 '+correct+'문제 정답!'+sharePerc+' 너도 도전해봐!':lang==='ja'?'🧮 計算60秒で'+correct+'問正解！'+(correct>=15?' 上位20%！':'')+' あなたも挑戦！':'🧮 Solved '+correct+' math problems in 60s!'+(correct>=20?' Top 5%!':'')+' Can you beat it?';window._shareResult={title:shtxt,text:shtxt,url:window.location.href};mathResult.textContent=txt.totalCorrect+correct+txt.problems+mathMsg;mathResult.style.color='#0a66c2';if(mathIsNew)mathResult.textContent+=(window.i18n[lang].newRecord||' 🎉 신기록!');if(historyVisible)renderHistory();mathStart.style.display='inline-block';}},1000);}
