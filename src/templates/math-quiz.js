...
module.exports = function(options) {
  ...
  var gameHTML = `
    ...
    <script>
      ...
      function startGame(){timeLeft=60;correct=0;correctEl.textContent=correct;mathInput.disabled=false;mathInput.value='';mathInput.focus();mathStart.style.display='none';mathResult.textContent='';generateQuestion();timerInterval=setInterval(function(){timeLeft--;timerEl.textContent=timeLeft;if(timeLeft<=0){clearInterval(timerInterval);mathInput.disabled=true;var lang=window.currentLang||'ko';var txt=window.i18n[lang].games.math;mathQuestion.textContent=txt.timeUp;window.GameRecord.save('math-quiz','correct',correct);if(correct>bestCorrect){bestCorrect=correct;bestCorrectEl.textContent=bestCorrect;}var shtxt=lang==='ko'?'🧮 암산 게임 60초에 '+correct+'문제 정답! 너도 도전해봐!':lang==='ja'?'🧮 計算ゲーム60秒で'+correct+'問正解！あなたも挑戦！':'🧮 I solved '+correct+' math problems in 60 seconds! Can you beat it?';window._shareResult={title:shtxt,text:shtxt,url:window.location.href};mathResult.textContent=txt.totalCorrect+correct+txt.problems;mathResult.style.color='#0a66c2';if(historyVisible)renderHistory();mathStart.style.display='inline-block';}},1000);}
