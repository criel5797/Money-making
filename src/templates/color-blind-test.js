  ...
  var gameHTML = `
    ...
    <script>
      ...
      function endGame(){
        ...
        var cbIsNew=window.GameRecord.save('color-blind-test','score',score);if(historyVisible)renderHistory();if(cbIsNew)cbResult.textContent+=(window.i18n[lang].newRecord||' 🎉 신기록!');var lang=window.currentLang||'ko';var shtxt=lang==='ko'?'👁️ 색맹 테스트 '+score+'점! 내 색각은?':lang==='ja'?'👁️ 色覚テスト'+score+'点！あなたも試して！':'👁️ Scored '+score+' on color vision test! How is yours?';window._shareResult={title:shtxt,text:shtxt,url:window.location.href};
