...
module.exports = function(options) {
  ...
  var gameHTML = `
    ...
    <script>
      ...
      function clearPendingTarget(){if(nextTargetTimeout){clearTimeout(nextTargetTimeout);nextTargetTimeout=null;}}
      function createTarget(){if(!started)return;clearPendingTarget();if(target)target.remove();target=document.createElement('div');var size=Math.random()*30+50;var maxX=aimArea.offsetWidth-size-10;var maxY=aimArea.offsetHeight-size-10;var x=Math.max(10,Math.random()*maxX);var y=Math.max(10,Math.random()*maxY);target.style.cssText='position:absolute;width:'+size+'px;height:'+size+'px;background:#e74c3c;border-radius:50%;cursor:pointer;transition:transform 0.1s;box-shadow:0 2px 8px rgba(231,76,60,0.4)';target.style.left=x+'px';target.style.top=y+'px';target.addEventListener('mouseenter',function(){this.style.transform='scale(1.1)';});target.addEventListener('mouseleave',function(){this.style.transform='scale(1)';});target.addEventListener('click',function(e){e.stopPropagation();if(!started)return;hits++;hitsEl.textContent=hits;aimAccuracyEl.textContent=Math.round(hits/(hits+misses)*100)+'%';this.style.background='#27ae60';this.style.transform='scale(0)';this.style.pointerEvents='none';nextTargetTimeout=setTimeout(function(){nextTargetTimeout=null;createTarget();},100);});aimArea.appendChild(target);}
      function startCountdown(){countdown=3;timeLeft=30;aimTimer.textContent=timeLeft;aimArea.innerHTML='<div style="font-size:72px;font-weight:bold;color:#667eea">'+countdown+'</div>';var countInterval=setInterval(function(){countdown--;if(countdown>0){aimArea.innerHTML='<div style="font-size:72px;font-weight:bold;color:#667eea">'+countdown+'</div>';}else{clearInterval(countInterval);countdown=null;aimArea.textContent='';started=true;hits=0;misses=0;timeLeft=30;hitsEl.textContent=hits;aimAccuracyEl.textContent='0%';createTarget();interval=setInterval(function(){timeLeft--;aimTimer.textContent=timeLeft;if(timeLeft<=0){clearInterval(interval);interval=null;started=false;clearPendingTarget();if(target){target.remove();target=null;}var lang=window.currentLang||'ko';var txt=window.i18n[lang].games.aim;var aimIsNew=window.GameRecord.save('aim-trainer','hits',hits);if(hits>bestHits){bestHits=hits;bestHitsEl.textContent=bestHits;}var acc=hits+misses>0?Math.round(hits/(hits+misses)*100):0;var aimMsg=hits>=30?txt.msg1:hits>=25?txt.msg2:hits>=20?txt.msg3:hits>=15?txt.msg4:hits>=10?txt.msg5:txt.msg6;var sharePerc=hits>=30?' 상위 1%!':hits>=25?' 상위 5%!':hits>=20?' 상위 20%!':'';var shtxt=lang==='ko'?'🎯 목표물 '+hits+'회 명중 / 정확도 '+acc+'%!'+sharePerc+' 너도 해봐!':lang==='ja'?'🎯 ターゲット'+hits+'回ヒット！精度'+acc+'%！'+(hits>=20?' 上位20%！':'')+' あなたも挑戦！':'🎯 Hit '+hits+' targets with '+acc+'% accuracy!'+(hits>=25?' Top 5%!':'')+' Can you beat it?';window._shareResult={title:shtxt,text:shtxt,url:window.location.href};if(historyVisible)renderHistory();var aimNewRec=aimIsNew?(window.i18n[lang].newRecord||' 🎉 신기록!'):'';aimArea.innerHTML='<span style="font-size:24px;color:#666">'+txt.complete+hits+txt.hits+acc+'%)'+aimMsg+aimNewRec+txt.clickAgain+'</span>';}},1000);}},1000);}
