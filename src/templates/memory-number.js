...
module.exports = function(options) {
  ...
  var gameHTML = `
    ...
    <script>
      ...
      function checkAnswer(){var lang=window.currentLang||'ko';var txt=window.i18n[lang].games.memoryNumber;if(input.value===currentNumber){var completedLevel=level;level++;levelEl.textContent=level;if(completedLevel>bestLevel){bestLevel=completedLevel;bestLevelEl.textContent=bestLevel;window.GameRecord.save('memory-number','level',bestLevel);}display.textContent=txt.correct;display.style.color='#27ae60';input.style.display='none';submitBtn.style.display='none';setTimeout(showNumber,1500);}else{var finalLevel=Math.max(level-1,0);window.GameRecord.save('memory-number','level',finalLevel);if(historyVisible)renderHistory();var shareMsg=lang==='ko'?'🧠 숫자 기억력 '+finalLevel+'단계 달성! 너도 도전해봐!':lang==='ja'?'🧠 数字記憶 '+finalLevel+'段階達成！あなたも挑戦！':'🧠 I memorized '+finalLevel+'-digit numbers! Can you beat it?';window._shareResult={title:shareMsg,text:shareMsg,url:window.location.href};display.textContent=txt.wrong+currentNumber;display.style.color='#e74c3c';level=1;levelEl.textContent=level;setTimeout(function(){display.textContent='';display.style.color='#333';startBtn.style.display='inline-block';input.style.display='none';submitBtn.style.display='none';state='ready';},3000);}input.style.display='none';submitBtn.style.display='none';}
