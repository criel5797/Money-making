...
module.exports = function(options) {
  ...
  var gameHTML = `
    ...
    <script>
      ...
      typingInput.addEventListener('input',function(){updateDisplay();if(typingInput.value===currentText){var elapsed=(Date.now()-startTime)/1000/60;var wpm=Math.round(currentText.length/5/elapsed);wpmEl.textContent=wpm;var finalAcc=Math.round(((currentText.length-errors)/currentText.length)*100);accuracyEl.textContent=finalAcc+'%';var lang=window.currentLang||'ko';var txt=window.i18n[lang].games.typing;var typingMsg=wpm>=80?txt.msg1:wpm>=60?txt.msg2:wpm>=50?txt.msg3:wpm>=40?txt.msg4:wpm>=30?txt.msg5:txt.msg6;typingResult.textContent=txt.complete+wpm+txt.accuracy+finalAcc+'%'+typingMsg;typingResult.style.color='#27ae60';window.GameRecord.save('typing-speed','wpm',wpm);if(wpm>bestWpm){bestWpm=wpm;bestWpmEl.textContent=wpm;}var sharePerc=wpm>=80?' 상위 1%!':wpm>=60?' 상위 5%!':wpm>=50?' 상위 20%!':'';var shareMsg=lang==='ko'?'⌨️ 타이핑 속도 '+wpm+' WPM! 정확도 '+finalAcc+'%!'+sharePerc+' 너도 도전해봐!':lang==='ja'?'⌨️ タイピング速度 '+wpm+' WPM！精度 '+finalAcc+'%！'+(wpm>=50?' 上位20%！':'')+' あなたも挑戦！':'⌨️ Typing speed: '+wpm+' WPM! Accuracy: '+finalAcc+'%!'+(wpm>=60?' Top 5%!':'')+' Can you beat it?';window._shareResult={title:shareMsg,text:shareMsg,url:window.location.href};if(historyVisible)renderHistory();typingInput.disabled=true;typingStart.style.display='inline-block';}});
    </script>
  `;
  return gameHTML;
};
