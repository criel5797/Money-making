...
module.exports = function(options) {
  ...
  var gameHTML = `
    ...
    <script>
      ...
      for(var i=0;i<9;i++){var btn=document.createElement('div');btn.style.cssText='width:100px;height:100px;background:#ecf0f1;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:32px;font-weight:bold;color:#34495e;cursor:pointer;transition:all 0.2s';btn.dataset.index=i;btn.addEventListener('click',function(){if(!playing)return;var idx=parseInt(this.dataset.index);this.style.background='#3498db';setTimeout(function(el){el.style.background='#ecf0f1';},200,this);playerSeq.push(idx);var lang=window.currentLang||'ko';var txt=window.i18n[lang].games.sequence;if(playerSeq[playerSeq.length-1]!==sequence[playerSeq.length-1]){seqResult.textContent=txt.wrong;seqResult.style.color='#e74c3c';playing=false;var seqIsNew=window.GameRecord.save('sequence-memory','level',Math.max(level-1,0));if(historyVisible)renderHistory();var seqFinalLvl=Math.max(level-1,0);var seqLang=window.currentLang||'ko';var seqTxt2=window.i18n[seqLang].games.sequence;var seqMsg=seqFinalLvl>=10?seqTxt2.msg1:seqFinalLvl>=7?seqTxt2.msg2:seqFinalLvl>=5?seqTxt2.msg3:seqFinalLvl>=3?seqTxt2.msg4:seqTxt2.msg5;seqResult.textContent=txt.wrong+seqMsg;if(seqIsNew)seqResult.textContent+=(window.i18n[seqLang].newRecord||' 🎉 신기록!');seqResult.style.color='#e74c3c';var seqShareMsg=seqLang==='ko'?'🔢 순서 기억 레벨 '+seqFinalLvl+' 달성!'+(seqFinalLvl>=10?' 상위 1%!':seqFinalLvl>=7?' 상위 5%!':'')+' 너도 도전해봐!':seqLang==='ja'?'🔢 シーケンスメモリ レベル'+seqFinalLvl+'達成！'+(seqFinalLvl>=7?' 上位5%！':'')+' あなたも挑戦！':'🔢 Sequence memory level '+seqFinalLvl+'!'+(seqFinalLvl>=10?' Top 1%!':seqFinalLvl>=7?' Top 5%!':'')+' Can you beat it?';window._shareResult={title:seqShareMsg,text:seqShareMsg,url:window.location.href};level=1;seqLevelEl.textContent=level;seqStart.style.display='inline-block';}else if(playerSeq.length===sequence.length){var completedLevel=level;playing=false;level++;seqLevelEl.textContent=level;if(completedLevel>bestLevel){bestLevel=completedLevel;seqBestEl.textContent=bestLevel;window.GameRecord.save('sequence-memory','level',bestLevel);}seqResult.textContent=txt.correct;seqResult.style.color='#27ae60';setTimeout(showSequence,1500);}});seqGrid.appendChild(btn);}
