...
module.exports = function(options) {
  ...
  var gameHTML = `
    ...
    <script>
      ...
      function startGame(){started=true;score=0;streak=0;timeLeft=60;scoreEl.textContent=score;streakEl.textContent=streak;yesBtn.disabled=false;noBtn.disabled=false;colorStart.style.display='none';colorResult.textContent='';nextRound();timerInterval=setInterval(function(){timeLeft--;colorTimer.textContent=timeLeft;if(timeLeft<=0){clearInterval(timerInterval);started=false;yesBtn.disabled=true;noBtn.disabled=true;var lang=window.currentLang||'ko';var txt=window.i18n[lang].games.colorMatch;colorWord.textContent=txt.timeUp;colorWord.style.color='#666';var cmIsNew=window.GameRecord.save('color-match','score',score);if(score>bestScore){bestScore=score;bestScoreEl.textContent=bestScore;}var shtxt=lang==='ko'?'🎨 색깔 맞추기 점수 '+score+'점! 너도 해봐!':lang==='ja'?'🎨 色合わせ'+score+'点！あなたも挑戦！':'🎨 I scored '+score+' on the color match game! Beat me!';window._shareResult={title:shtxt,text:shtxt,url:window.location.href};var cmMsg=score>=40?txt.msg1:score>=30?txt.msg2:score>=20?txt.msg3:score>=10?txt.msg4:score>=5?txt.msg5:txt.msg6;colorResult.textContent=txt.totalScore+score+txt.points+cmMsg;colorResult.style.color='#0a66c2';if(cmIsNew)colorResult.textContent+=(window.i18n[lang].newRecord||' 🎉 신기록!');if(historyVisible)renderHistory();colorStart.style.display='inline-block';}},1000);}
